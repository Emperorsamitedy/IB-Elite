...
export function ScanUploader({ questions }: { questions: QuestionOption[] }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight">Scan your answer</h1>
              ...
              <ul className="flex flex-col gap-1 text-sm">
                {scan.annotationResult.markPoints.map((point, index) => (
                  <li key={`${point.text}-${index}`} className="flex gap-2">
                    <span
                      className={
                        point.present ? "text-success" : "text-danger"
                      }
                    >
                      {point.present ? "✓" : "✗"}
                    </span>
