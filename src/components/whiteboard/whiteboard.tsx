...
const STORAGE_KEY = "atlas:whiteboard:v1";
...
function readStoredBoard(): WB.Board | null {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<WB.Board>;
    if (!Array.isArray(parsed.strokes) || !Array.isArray(parsed.items)) {
      return null;
    }
    return {
...
export function Whiteboard({ providerLabel }: { providerLabel: string }) {
  const { resolvedTheme } = useTheme();

  const [board, setBoard] = React.useState<WB.Board>(WB.EMPTY_BOARD);
  const [past, setPast] = React.useState<WB.Board[]>([]);
  const [future, setFuture] = React.useState<WB.Board[]>([]);
  ...
  const commit = React.useCallback((next: WB.Board) => {
    setPast((p) => [...p, boardRef.current].slice(-HISTORY_LIMIT));
    setFuture([]);
    setBoard(next);
  }, []);
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        toast.error("This board is too big to save in your browser.", {
          description: "Export it as a PNG to keep a copy.",
        });
      }
    }, 400);
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (tool === "move") {
      ...
      setPast((p) => [...p, board].slice(-HISTORY_LIMIT));
      setFuture([]);
      return;
    }
  ...
  const endPointer = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    setBoard((current) => ({
      ...current,
      strokes: [...current.strokes, drawing.stroke],
    }));
  };
  ...
  const onPickPhoto = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const { src, width, height } = await WB.compressImage(file);
      await WB.loadImage(src);
      ...
      commit({
        ...
        items: [
          ...board.items,
          { id: WB.newId(), kind: "image", src, ...placement },
        ],
      });
      await runScan(src, "photo");
    } catch {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs text-muted-foreground">
        Saved in this browser automatically · Scans read by {providerLabel} ·
