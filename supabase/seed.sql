-- =============================================================
-- Seed / demo content — original practice questions written for
-- this platform. Clearly marked as seed data (source column).
-- Safe to re-run: uses stable slugs + upserts.
-- =============================================================

-- ---------- subjects ----------
insert into public.subjects (slug, name, group_name, description, color, sort_order) values
  ('math-aa', 'Mathematics: Analysis & Approaches', 'Mathematics', 'Algebra, functions, calculus, and proof with an emphasis on analytical methods.', '#6366f1', 1),
  ('math-ai', 'Mathematics: Applications & Interpretation', 'Mathematics', 'Modelling, statistics, and applied mathematics with technology.', '#8b5cf6', 2),
  ('physics', 'Physics', 'Sciences', 'Mechanics, fields, waves, and modern physics.', '#0ea5e9', 3),
  ('chemistry', 'Chemistry', 'Sciences', 'Structure, bonding, energetics, and reactivity.', '#10b981', 4),
  ('biology', 'Biology', 'Sciences', 'Cells, genetics, physiology, and ecology.', '#22c55e', 5),
  ('economics', 'Economics', 'Individuals & Societies', 'Micro, macro, and the global economy.', '#f59e0b', 6)
...
cross join (values ('HL', 'Higher Level', 1), ('SL', 'Standard Level', 2)) as l(code, name, sort_order)
...
from public.subjects s
join (values
  ('math-aa', 'number-algebra', 'Number & Algebra', 'Sequences, exponents, logarithms, binomial theorem, and proof.', 1),
  ('math-aa', 'functions', 'Functions', 'Transformations, quadratics, rational and composite functions.', 2),
  ('math-aa', 'geometry-trig', 'Geometry & Trigonometry', 'Triangles, identities, equations, and vectors.', 3),
  ('math-aa', 'statistics-probability', 'Statistics & Probability', 'Distributions, conditional probability, and expectation.', 4),
  ('math-aa', 'calculus', 'Calculus', 'Differentiation, integration, and their applications.', 5),
  ('math-ai', 'number-finance', 'Number & Finance', 'Percentages, sequences, and financial mathematics.', 1),
  ('math-ai', 'statistics', 'Statistics', 'Data analysis, correlation, and the chi-squared test.', 2),
  ('math-ai', 'modelling', 'Modelling', 'Linear, exponential, and trigonometric models.', 3),
  ('physics', 'mechanics', 'Mechanics', 'Kinematics, forces, momentum, and energy.', 1),
  ('physics', 'fields', 'Fields', 'Gravitational, electric, and magnetic fields.', 2),
  ('physics', 'waves', 'Waves', 'Oscillations, wave behaviour, and interference.', 3),
  ('chemistry', 'stoichiometry', 'Stoichiometry', 'The mole, equations, and reacting quantities.', 1),
  ('chemistry', 'bonding', 'Structure & Bonding', 'Atomic structure, bonding, and intermolecular forces.', 2),
  ('chemistry', 'energetics', 'Energetics', 'Enthalpy, Hess''s law, and bond energies.', 3),
  ('biology', 'cells', 'Cell Biology', 'Cell structure, membranes, and transport.', 1),
  ('biology', 'genetics', 'Genetics', 'DNA, inheritance, and biotechnology.', 2),
  ('biology', 'physiology', 'Human Physiology', 'Digestion, gas exchange, and homeostasis.', 3),
  ('economics', 'microeconomics', 'Microeconomics', 'Demand, supply, elasticity, and market failure.', 1),
  ('economics', 'macroeconomics', 'Macroeconomics', 'Growth, inflation, unemployment, and policy.', 2),
  ('economics', 'global-economy', 'The Global Economy', 'Trade, exchange rates, and development.', 3)
) as t(subject_slug, slug, name, description, sort_order) on t.subject_slug = s.slug
...
join (values
  ('math-aa', 'number-algebra', 'sequences', 'Sequences & Series', 1),
  ('math-aa', 'number-algebra', 'logs-exponents', 'Logarithms & Exponents', 2),
  ('math-aa', 'functions', 'quadratics', 'Quadratics', 1),
  ('math-aa', 'functions', 'transformations', 'Transformations', 2),
  ('math-aa', 'geometry-trig', 'identities', 'Trigonometric Identities', 1),
  ('math-aa', 'geometry-trig', 'vectors', 'Vectors', 2),
  ('math-aa', 'calculus', 'differentiation', 'Differentiation', 1),
  ('math-aa', 'calculus', 'integration', 'Integration', 2)
) as st(subject_slug, topic_slug, slug, name, sort_order)
  on st.subject_slug = s.slug and st.topic_slug = tp.slug
...
from (values
  ...
  ('math-aa','number-algebra','HL','medium',4,'short-answer',false,
   'Solve for x: log_2(x) + log_2(x - 2) = 3.',
   'x = 4','log_2(x(x-2)) = 3 ⇒ x^2 - 2x = 8 ⇒ x^2 - 2x - 8 = 0 ⇒ (x-4)(x+2)=0. x>2 so x = 4.'),
  ('math-aa','number-algebra','HL','hard',6,'short-answer',false,
  ...
  ('math-aa','functions','SL','medium',4,'short-answer',false,
  ...
  ('physics','fields','HL','hard',6,'long-answer',true,
  ...
  ('chemistry','energetics','HL','hard',6,'long-answer',true,
  ...
  -- biology
  ('biology','cells','SL','easy',2,'multiple-choice',false,
  ...
  ('biology','genetics','HL','medium',4,'short-answer',false,
  ...
  ('biology','physiology','SL','medium',4,'short-answer',false,
  ...
  -- economics
  ('economics','microeconomics','SL','medium',4,'long-answer',false,
  ...
  ('economics','macroeconomics','HL','medium',5,'long-answer',false,
  ...
  ('economics','global-economy','HL','hard',6,'long-answer',false,
